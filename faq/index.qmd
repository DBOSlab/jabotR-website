...
### Can jabotR assess how well my herbarium is represented, or find missing species?

Yes. Beyond retrieving and filtering records, `jabotR` includes three analytical functions that compare any JABOT-hosted herbarium against the [Flora e Funga do Brasil (FFB)](https://floradobrasil.jbrj.gov.br/consulta/): `jabot_gaps()` identifies taxa missing from a collection, `jabot_coverage()` measures how much of the FFB checklist a herbarium already represents, and `jabot_missing()` narrows that comparison to a specific state or municipality and maps candidate collecting localities from the pooled JABOT network. Each produces a self-contained, interactive HTML report. See the [How-To Articles](/articles/index.qmd) for step-by-step tutorials.
