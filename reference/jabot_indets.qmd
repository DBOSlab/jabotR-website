---
title: 'jabot_indets'
description: 'Retrieve indeterminate specimens from JABOT'
toc: true
toc-depth: 3
---
...
only to family or genus level) from the [JABOT](https://ipt.jbrj.gov.br/jabot)
...
like `.arg_check_herbarium()` and `.filter_occur_df()` to validate
inputs and refine the occurrence records. If `path` is not provided, the
...
results will be written to the specified `dir`, and a `log.txt` file
